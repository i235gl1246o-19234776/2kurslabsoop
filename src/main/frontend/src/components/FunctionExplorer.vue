    ...
    <div class="modal-content explorer-modal">
      ...
      <div v-else>
        ...
        <div class="points-table">
          ...
          <!-- Горизонтальный ползунок для навигации по точкам -->
...
const visiblePoints = computed(() => {
  ...
  // Если используем диапазон от ползунка
  if (visibleRange.value.end > 0) {
    const end = Math.min(visibleRange.value.end + 1, points.value.length)
    return points.value.slice(visibleRange.value.start, end)
  }

  // По умолчанию возвращаем все точки или первую страницу
  const end = Math.min(maxVisiblePoints.value, points.value.length)
  return points.value.slice(0, end)
})
...
// Отслеживание изменений в props для обновления точек
...
const insertPoint = () => {
  ...
  // Вставка точки с сохранением сортировки по x
  const index = points.value.findIndex(p => p.x > newPoint.value.x)
  if (index === -1) {
    points.value.push({ ...newPoint.value })
  } else {
    points.value.splice(index, 0, { ...newPoint.value })
  }
...
const removePoint = (index) => {
  if (confirm('Вы уверены, что хотите удалить эту точку?')) {
    ...
    // Корректировка пагинации при необходимости
...
const handlePointSelected = (pointData) => {
  highlightedIndex.value = pointData.index + visibleRange.value.start
...
const handleRangeChanged = (range) => {
  ...
  currentPage.value = 1 // Сбрасываем пагинацию при изменении диапазона
}

const decreasePage = () => {
  if (currentPage.value > 1) {
    currentPage.value--
    visibleRange.value = { start: 0, end: 0 } // Сбрасываем диапазон при использовании пагинации
  }
}
